...
// the minimum amount of time the tip needs to be depressed (in contact) for
// sabre 0.1ms -> 1ms These values are stored as micro seconds for more accuracy
//                         foil   epee   sabre
...
//============
void loop() {
   // use a while as a main loop as the loop() has too much overhead for fast
   // analogReads we get a 3-4% speed up on the loop this way
   while (1) {
      checkIfModeChanged();
      // read analog pins
...
void foil_sub(bool &hitOnTarget, bool &hitOffTarget, bool &depressed, int &weaponAnalog, int &lameAnalog, long &depressedTime) {
   ...
   if (900 < weaponAnalog && lameAnalog < 100) {
      ...
      // on target
      if (450 < weaponAnalog && weaponAnalog < 600 && 300 < lameAnalog && lameAnalog < 600) {
         if (!depressed) {
            depressedTime = micros();
            depressed = true;
         } else {
...
void sabre() {
   long now = micros();
   if ((hitOnTargA && (depressAtime + lockout[2] < now)) ||
       (hitOnTargB && (depressBtime + lockout[2] < now))) {
      lockedOut = true;
   }
